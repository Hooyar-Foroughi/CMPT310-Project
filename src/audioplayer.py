...
def audioplayer(file):
    ...
    # file=sys.argv[1]
    assert(os.path.exists(Path(AUDIO_DIRECTORY)/(Path(file).stem+".wav"))) # Verify that an .wav file with same name exists
    ...
    wav = preprocess_wav(Path(AUDIO_DIRECTORY)/(Path(file).stem+".wav"))   # Obtain VAD audio so the timestamps sync up
