...
def maketimestamp(path, model="ag"):
    ...
    if model=="sp":
        outputdirectory = Path(path).parent.parent/'sptimestamp'
    else:
        outputdirectory = Path(path).parent.parent/'agtimestamp'
    ...
    timestamp=pd.DataFrame(columns=['speaker','start','end'])
